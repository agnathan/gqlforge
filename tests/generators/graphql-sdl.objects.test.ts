/**
 * GraphQL SDL Generator Tests - 3.6 Object Type Definition
 * 
 * Tests for Object Type Definition generation (section 3.6 of GraphQL spec).
 */
...
describe("graphql-sdl generator - 3.6 Object Type Definition", () => {
  ...
  describe("3.6 Object Type Definition", () => {
    ...
    registerTestID(TEST_ID, "tests/generators/graphql-sdl.objects.test.ts", "generates object type definition with fields");

    it(`[${TEST_ID}] generates object type definition with fields`, async () => {
      const input = fixtures.input("GEN-GSDL-003-generates-object-type-definition-with-fields-grammar.json") as Grammar;
      const expected = fixtures.expected("GEN-GSDL-003-generates-object-type-definition-with-fields-grammar.graphql");
        ...
        validationResult = await expectValidGraphQL(output, undefined, {
          ...
          saveOutput:
            process.env.SAVE_TEST_OUTPUTS === "true"
                ? "GEN-GSDL-003-generates-object-type-definition-with-fields-grammar.graphql"
              : false,
